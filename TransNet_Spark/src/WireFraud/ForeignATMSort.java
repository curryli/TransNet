//hadoop jar TeleTrans2.jar  WireFraud.ForeignATMSort -Dmapreduce.job.queuename=root.default TeleTrans/quxian_1606 TeleTrans/saveForeignATM_1606
//hadoop jar TeleTrans2.jar  WireFraud.ForeignATMSort -Dmapreduce.job.queuename=root.spark TeleTrans/quxian_1606 TeleTrans/saveFATM_5000_1606 
			...
			for (Text info : lines) {
				if(!info.toString().isEmpty()){
	              ...
	              if(curTS.length()==14 && trans_md.equals("2") && Double.valueOf(money)>=500000){	
				    ...
				    String tempInfo = ATM + "\t" + pdate + "\t" + money + "\t"+ card + "\t" + isCross + "\t" +trans_md + "\t" + DeltaT;
