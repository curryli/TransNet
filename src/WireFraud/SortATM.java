//hadoop jar TeleTrans.jar  WireFraud.SortATM -Dmapreduce.job.queuename=root.default TeleTrans/quxian_1606 TeleTrans/saveATM_1606
...
public class SortATM {
	...
	public static class UpdateMapper extends Mapper<LongWritable, Text, MultiSortUtil.KeyPair, Text> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if(list.length == 9){	
            	...
            	String money = list[1];
            	String newstr = ATM + " " + time + " " + money + " " + card + " " + isCross;
            	info.set(newstr);
		...
		public void reduce(MultiSortUtil.KeyPair key, Iterable<Text> lines, Context context)
			...
			for (Text info : lines) {
	              ...
	              if(curTS.length()==14){	
				    ...
				    String tempInfo = ATM + "\t" + pdate + "\t" + money + "\t"+ card + "\t" + isCross + "\t" + DeltaT;
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job(conf,"SortATM");

		job.setJarByClass(SortATM.class); // 设置运行jar中的class名称
