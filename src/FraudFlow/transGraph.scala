...
object transGraph { 
  def main(args: Array[String]) { 
    ...
    connectedCount.filter(f => f._2 >=3).collect().foreach{f=>
         ...
         subG.vertices.saveAsTextFile("xrli/TeleFraud/vertices/v_" + f._2 + "_" + label)
         subG.edges.saveAsTextFile("xrli/TeleFraud/edges/e_" + f._2 + "_"+ label)
